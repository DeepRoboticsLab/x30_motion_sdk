...
当SDK没有指令下发超过1s时，底层控制器会拿回控制权，进入阻尼保护模式一段时间后，清空关节指令。控制流程可以参考下图：
...
| 膝关节(Knee) | 24°~145°     | 65Nm         | 11rad/s      | 180Nm        |
...
- 修改配置文件第4行`ips`中的第3个IP地址，使得 **MotionSDK** 能够接收到机器狗数据:
	- 如果 **MotionSDK** 在机器狗运动主机内运行，IP设置为运动主机IP：`192.168.1.103`；  
...
- 在同一个配置文件 ***sdk_config.toml*** 中，可按需对关节力矩限幅进行修改：
	...
	| 数组元素 | 含义     | 取值范围  |数据类型|
	| --- | -------- | ---------- | ---------- |
	| torque_limit[0] | 对髋关节hipx的力矩限幅(N)| (0,84.0] | double |
	| torque_limit[1] | 对侧摆关节hipy的力矩限幅(N)| (0,84.0] | double |
	| torque_limit[2] | 对膝关节knee的力矩限幅(N)| (0,160.0] | double |
...
- 编译结束后，会在 ***build*** 目录下生成一个名为 ***X30_motion*** 的可执行文件，运行该文件时，机器狗将会执行下发的控制指令：
	...
	./X30_motion
	```
> **注意：用户在使用X30执行算法和实验的过程中，请与机器狗保持至少5米距离，并将机器狗悬挂在调试架上避免意外造成人员和设备损伤。若实验过程中，机器狗摔倒或者用户想搬动机器狗位置，用户想要靠近机器狗时，用户应当使得机器狗处于急停状态或者关闭运动程序（使用 `sudo ./stop.sh` 命令）**
